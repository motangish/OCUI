...
--SHELL ARGS
local shellArgs, shellOptions = shell.parse(...)
...
local function newFile()
    ...
    local function done()
        ...
        if w and h then
            if not canvas then
                canvas = ui.canvas(0, 0, 0, 0xFFFFFF, " ", image.new("canvas", w, h))
            ...
            canvas.x, canvas.y = ui.centerSquare(w + 1, h + 1)
...
local function openFile(path)
    ...
    local function done()
        local text = path or pathTB.text
        if fs.exists(text) then
            ...
            canvas = ui.canvas(0, 0, currBColor or 0, currTColor or 0xFFFFFF, currSymbol or " ", image.load(text))
            canvas.x, canvas.y = ui.centerSquare(canvas.image.width + 1, canvas.image.height + 1)
...
local function saveFile(type)
    if canvas then
        if type == 0 and shellArgs[1] then
            canvas.image:save(shellArgs[1])
        else
...
local function init()
    ...
    fileCM:addObj("Сохранить", saveFile, 0)
    fileCM:addObj("Сохранить как", saveFile, 1)
    bar:addObj(fileCM)
    ...
    bar:addObj(editCM)
    mainBox:addObj(bar)
    if shellArgs[1] then openFile(shellArgs[1]) end
end
